...
capture = cv2.VideoCapture('D:/KULIAH/Hasil Deteksi/Speed Estimation/Hasil/8/8_kanankekiri.mp4')

colors = [tuple(255 * np.random.rand(3)) for i in range(5)]
frame_width = int(capture.get(3)) #Mengambil ukuran width video input
frame_height = int(capture.get(4)) #Mengambil ukuran height video input
out = cv2.VideoWriter('D:/KULIAH/Hasil Deteksi/Speed Estimation/Hasil/8/8_kanankekiri.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 25, (frame_width,frame_height)) #Untuk menyimpan hasil deteksi ke dalam video

#export to excel (deklarasi header tabel excel)
workbook = xlsxwriter.Workbook('D:/KULIAH/Hasil Deteksi/Speed Estimation/Hasil/8/8_kanankekiri.xlsx') 
...
distance = 83 #Approx real distance in a video frame from every edge
meterPerPixel = distance/frame_width #Calculate meter per pixel
condition_dir = frame_width/2
...
while (capture.isOpened()):
    ...
    if ret:
        ...
        for color, result in zip(colors, results):
            ...
            #print(centerCoord_column)
            #print(estimated)
            #Perhitungan Speed Estimation Object
            if not estimated:
                if (direction == 0) and (float(centerCoord[0]) < 930):
                ...
                if lastPoint and not estimated: #jika nilai lastPoint true maka masuk ke kondisi ini
                    ...
                    print('{} -> Objek ke {}'.format(speed, row2-1)) #melakukan print di command prompt
            else:
                #print("Masuk else")
                reset_variable()

            text = '{}: {:.0f}%'.format(label, confidence * 100)
            fps = '{:.2f}'.format(1 / (time.time() - stime))
        ...
        #cv2.imshow('frame', frame) #Untuk menampilkan video hasil deteksi secara live pada window
